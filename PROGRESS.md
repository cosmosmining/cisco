...
## Phase 6 (stretch) — Robustness & security pass (2026-06-09)

Built: scapy mutational fuzzer (`test/fuzz/fuzz_scapy.py` — truncation,
length-field stomps, bit flips, bogus IHL/doff/options, flag soup,
overlapping fragments, garbage; liveness-checked against the live ASan
stack), AFL++ persistent-mode harness over the full parser chain
(`fuzz/parse_harness.c` — two-interface forwarding stack with live UDP/TCP
listeners, fed raw frames; `fuzz/run_afl.sh`; corpus generated by
`fuzz/make_corpus.py`; `make fuzz-replay` regression-runs the corpus),
coverage build + CI job publishing a gcovr HTML report, 3 new active-open
TCP unit tests (handshake, refused, simultaneous open) closing the
SYN_SENT coverage gap.

**Gate 6 evidence**

```
$ sudo python3 test/fuzz/fuzz_scapy.py --iterations 20000 --seed 1
OK: 20000 mutated frames in 30s, no crashes, stack still answers ping
$ sudo python3 test/fuzz/fuzz_scapy.py --iterations 80000 --seed 2
OK: 80000 mutated frames in 110s, no crashes, stack still answers ping
        → 100,000 frames total against build-asan/bin/pfstack, 0 crashes

$ ./fuzz/run_afl.sh 600          # AFL++ 4.09c, ASan-instrumented
run_time          : 600
execs_done        : 30380906     (≈50,634 execs/sec, persistent mode)
corpus_count      : 119          (from 13 seeds)
saved_crashes     : 0
saved_hangs       : 0

$ gcovr -r . build-cov --filter src/ --print-summary
lines: 87.0% (1868 out of 2148)      # unit + integration + corpus replay
functions: 96.6% (169 out of 175)
branches: 68.9% (891 out of 1294)
   per parser: checksum 100%, arp 95%, fib 95%, ip_reass 94%, tcp_out 93%,
   ipv4 92%, udp 91%, tcp 85%
```

Zero crashes or sanitizer reports across both recorded fuzz runs. The one
memory bug of the whole project so far (use-after-free in tcp_ooo_drain)
had already been caught by ASan via unit tests in phase 4 — the fuzzers
found nothing new to shake out.

## Phase 7 (stretch) — Embedded tie-in: descoped

The RISC-V/QEMU virtio-net port is prepared for but not implemented in
this pass (see DECISIONS.md D-019). The portability boundary it needs is
real and enforced today: no Linux headers or syscalls anywhere in
eth/arp/ipv4/icmp/udp/tcp/route/fwd — platform contact is limited to
`pf_now_ms()` and `netdev->tx`, the same seams the unit tests fake.
